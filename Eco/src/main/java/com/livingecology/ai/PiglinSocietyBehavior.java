...
/**
 * Low-touch overlay for living Piglin society.
 *
 * <p>Piglin and Piglin Brute Brain activities, bartering, fear, hunting, target selection and
 * navigation remain vanilla-owned. Living Ecology records territory/context and may transmit a
 * bounded local threat memory between living Piglins, but never assigns a target, avoid target or
 * path. Zombified Piglins deliberately remain in the separate undead-combat controller.</p>
 */
public final class PiglinSocietyBehavior {
    private static final int MAX_SOCIETY_MEMORY_RECEIVERS = 8;
    private static final double SOCIETY_MEMORY_RANGE = 16.0D;

    private PiglinSocietyBehavior() {}

    public static void tick(Mob mob, ServerLevel level) {
        SpeciesType species = SpeciesType.from(mob).orElse(null);
        if (species == null) return;
        PiglinSocietySpeciesPolicy policy = PiglinSocietySpeciesPolicy.of(species).orElse(null);
        if (policy == null || !policy.livingSociety()) return;
        ...
        shareBoundedSocietyMemory(mob, level, policy, report);
    }
    ...
    private static void shareBoundedSocietyMemory(Mob source, ServerLevel level,
                                                  PiglinSocietySpeciesPolicy sourcePolicy,
                                                  LivingEntity threat) {
        int informed = 0;
        for (Mob ally : level.getEntitiesOfClass(Mob.class,
                new AABB(source.blockPosition()).inflate(SOCIETY_MEMORY_RANGE), other -> {
                    ...
                    return PiglinSocietySpeciesPolicy.of(otherSpecies)
                            .filter(PiglinSocietySpeciesPolicy::livingSociety)
                            .map(policy -> policy.knowledgeGroup() == sourcePolicy.knowledgeGroup())
                            .orElse(false);
                })) {
            if (informed >= MAX_SOCIETY_MEMORY_RECEIVERS) break;
