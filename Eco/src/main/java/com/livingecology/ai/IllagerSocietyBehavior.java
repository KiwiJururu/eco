...
/**
 * Low-touch overlay for raid-capable Illager society.
 *
 * <p>Patrol, raid, spell, crossbow, potion, rider and role-specific combat/navigation remain
 * vanilla-owned. Living Ecology records local territory/environment context and can transmit a
 * bounded threat memory across Illager roles, but it never assigns a target or path.</p>
 */
public final class IllagerSocietyBehavior {
    private static final int MAX_MEMORY_RECEIVERS = 8;
    private static final double MEMORY_RANGE = 16.0D;

    private IllagerSocietyBehavior() {}

    public static void tick(Mob mob, ServerLevel level) {
        SpeciesType species = SpeciesType.from(mob).orElse(null);
        if (species == null) return;
        IllagerSocietySpeciesPolicy policy = IllagerSocietySpeciesPolicy.of(species).orElse(null);
        ...
        shareBoundedMemory(mob, level, policy, report);
    }
    ...
    private static void shareBoundedMemory(Mob source, ServerLevel level,
                                           IllagerSocietySpeciesPolicy sourcePolicy,
                                           LivingEntity threat) {
        int informed = 0;
        for (Mob ally : level.getEntitiesOfClass(Mob.class,
                new AABB(source.blockPosition()).inflate(MEMORY_RANGE), other -> {
                    ...
                    return otherSpecies != null && IllagerSocietySpeciesPolicy.of(otherSpecies)
